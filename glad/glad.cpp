/*
    ...
    OpenGL loader generated by glad 0.1.18a0 on Mon Apr  9 01:51:42 2018.
    ...
    Commandline:
        --profile="compatibility" --api="gl=3.3" --generator="c" --spec="gl" --local-files
   --omit-khrplatform --extensions="" Online:
...
#ifndef GLAD_ORIGINAL_CODE
void glad_safe_strcpy(char *dest, size_t dest_sz, char const *src, size_t src_sz)
{
    size_t safe_copy_rng = dest_sz - 1;
    if(src_sz < safe_copy_rng) {
        safe_copy_rng = src_sz;
    }

    size_t i = 0;
    for(; i < safe_copy_rng && src[i] != '\0'; ++i) {
        dest[i] = src[i];
    }
...
static int get_exts(void)
{
    ...
    if(max_loaded_major < 3) {
        ...
        for(index = 0; index < (unsigned)num_exts_i; index++) {
            ...
            char *local_str = (char *)malloc(dest_len * sizeof(char));
            if(local_str != NULL) {
#ifdef GLAD_ORIGINAL_CODE
#if _MSC_VER >= 1400
                strncpy_s(local_str, len + 1, gl_str_tmp, len);
#else
                strncpy(local_str, gl_str_tmp, dest_len);
#endif
#else // GLAD_ORIGINAL_CODE
                glad_safe_strcpy(local_str, dest_len, gl_str_tmp, len);
#endif
            }
